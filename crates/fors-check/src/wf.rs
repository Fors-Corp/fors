        ...
        match self.fir.tys.tag(ty) {
            ...
            TyTag::Proj => {
                let (tref, name) = self
                    .fir
                    .tys
                    .proj_key(fors_fir::ty::ProjKeyId(self.fir.tys.b(ty)));
    ...
    fn impl_holds(&mut self, subject: TyId, want: TraitRefId, want_def: DefId) -> Holds {
        ...
        for row in bucket {
            ...
            if one_way_match(&mut self.fir.tys, row.self_ty, subject, &mut b) {
                ...
                let wa = if want_args == NO_ARGS {
                    Vec::new()
                } else {
                    self.fir.tys.args_vec(want_args)
                };
                if ra.len() != wa.len() {
                    continue;
                }
...
impl Wf<'_> {
    ...
    fn marker_traits(&mut self, low: &Lowered, files: &[FileCtx]) {
        ...
        for r in rows {
            ...
            let Some((file, range)) = self.head_range(files, r.def) else {
                continue;
            };
    ...
    fn impl_params(&mut self, low: &Lowered, files: &[FileCtx]) {
        ...
        for r in rows {
            ...
            if r.inherent
                && matches!(
                    self.fir.tys.tag(r.self_ty),
                    TyTag::Fn | TyTag::Dyn | TyTag::Tuple
                )
            {
                self.emit(
                    r.def,
                    ...
                    18,
                    18,
                    "an inherent impl's type must be a struct or an enum".to_string(),
                );
                continue;
            }
            ...
            if head_tys
                .iter()
                .any(|&h| impls::contains_proj(&self.fir.tys, h))
            {
                self.emit(
                    r.def,
                    file,
                    ...
                    18,
                    18,
                    "an impl head must not contain a projection".to_string(),
                );
                continue;
            }
            ...
            for o in 0..n {
                ...
                let in_head = head_tys
                    .iter()
                    .any(|&h| impls::mentions(&self.fir.tys, h, want, &mut (1 << 16)));
                if !in_head {
                ...
                if bounded && !impls::mentions(&self.fir.tys, r.self_ty, want, &mut (1 << 16)) {
                    ...
                    self.emit(
                        ...
                        18,
                        18,
                        format!("bounded impl parameter `{nm}` does not occur in the self type"),
                    );
                    break;
                }
            }
    ...
    fn overlap(&mut self, low: &Lowered, files: &[FileCtx]) {
        ...
        for (i, r) in rows.iter().enumerate() {
            ...
            buckets
                .entry((r.trait_def.0, r.head.as_u64()))
                .or_default()
                .push(i);
        }
        ...
        for k in keys {
            ...
            for a in 0..ids.len() {
                for b in a + 1..ids.len() {
                    ...
                    let Some((file, range)) = self.head_range(files, late.def) else {
                        continue;
                    };
                    ...
                    self.emit(
                        ...
                        19,
                        19,
                        format!("this impl of `{nm}` overlaps an earlier one: their heads unify"),
                    );
                }
            }
        }
    ...
    fn impl_completeness(&mut self, low: &Lowered, files: &[FileCtx]) {
        ...
        for r in rows {
            ...
            if r.inherent {
                ...
                if !assoc_defined.is_empty() {
                    self.emit(
                        ...
                        17,
                        17,
                        "an inherent impl must not contain a `type` item".to_string(),
                    );
                }
                continue;
            }
            ...
            let site = if is_operator_trait(self.prelude, r.trait_def) {
                21
            } else {
                17
            };
            ...
            for &(name, _) in &declared {
                if !assoc_defined.iter().any(|&(n, _)| n == name) {
                    ...
                    self.emit(
                        ...
                        17,
                        site,
                        format!("this impl does not define the associated type `{nm}`"),
                    );
                    break;
                }
            }
            ...
            for &(name, _) in &assoc_defined {
                if !declared.iter().any(|&(n, _)| n == name) {
                    ...
                    self.emit(
                        ...
                        17,
                        site,
                        format!("`{nm}` is not an associated type of `{tn}`"),
                    );
                    break;
                }
            }
            ...
            for &(name, bounds) in &declared {
                let Some(&(_, rhs)) = assoc_defined.iter().find(|&&(n, _)| n == name) else {
                    continue;
                };
                ...
                for b in bs {
                    if self.holds(rhs, b) == Holds::No {
                        ...
                        let bsite = if self.is_language_known(self.fir.tys.trait_ref(b).0)
                            && r.trait_def == self.prelude.traits[tr::ITERATOR]
                        {
                            21
                        } else {
                            site
                        ...
                        self.emit(
                            ...
                            bcode,
                            bsite,
                            format!("`type {an} = ...;` does not satisfy the trait's bound `{bn}`"),
                        );
                        break;
                    }
                }
            ...
            for (name, _) in &required {
                if !defined.iter().any(|&(n, _)| n == *name) {
                    ...
                    self.emit(
                        ...
                        17,
                        site,
                        format!("this impl does not define the required method `{nm}`"),
                    );
                    break;
                }
            }
            ...
            for (name, _) in &defined {
                if !all_trait_methods.contains(name) {
                    ...
                    self.emit(
                        ...
                        17,
                        site,
                        format!("`{nm}` is not a method of `{tn}`"),
                    );
                    break;
                }
            }
        ...
        for i in 0..n {
            ...
            let provided = self
                ...
                .and_then(|row| {
                    files
                        .get(row.file.index())
                        .map(|f| f.has_block(row.node as usize))
                })
                .unwrap_or(false);
            if !provided {
...
impl Wf<'_> {
    ...
    fn method_signatures(
        ...
        for i in 0..n {
            ...
            let Some(&(_, idef)) = defined.iter().find(|&&(nm, _)| nm == m.name) else {
                continue;
            };
            if idef == NO_DEF {
                continue;
            ...
            let Some(sub) = self.trait_binding(r) else {
                continue;
            };
            ...
            if wn != gn {
                ...
                self.emit(
                    ...
                    17,
                    site,
                    format!("`{nm}` takes {gn} parameter(s); the trait declares {wn}"),
                );
                return;
            }
            ...
            if wgn != ggn {
                mismatch = Some(format!(
                    "it declares {ggn} generic parameter(s); the trait's declares {wgn}"
                ));
            } else {
                for i in 0..wgn {
                    ...
                    if a.kind.tag() != b.kind.tag() {
                        mismatch = Some(format!(
                            "generic parameter {} has a different kind from the trait's",
                            i + 1
                        ));
                        break;
                    }
                    ...
                    let heads = |this: &Self, xs: &[TraitRefId]| -> Vec<DefId> {
                        let mut v: Vec<DefId> =
                            xs.iter().map(|&t| this.fir.tys.trait_ref(t).0).collect();
                        v.sort();
                        v.dedup();
                        v
                    };
                    if heads(self, &ab) != heads(self, &bb) {
                        mismatch = Some(format!(
                            "generic parameter {}'s bounds are not the trait's",
                            i + 1
                        ));
                        break;
                    }
                ...
                if mismatch.is_none() {
                    let wc = self
                        ...
                        .sigs
                        .constraints
                        .count(self.fir.sigs.generics_store.constraints(wg));
                    let gc = self
                        .fir
                        .sigs
                        .constraints
                        .count(self.fir.sigs.generics_store.constraints(gg));
                    if wc != gc {
            ...
            for p in 0..wn {
                ...
                if wp.name != gp.name {
                    ...
                    mismatch = Some(format!(
                        "parameter {} is named `{b}`; the trait names it `{a}`",
                        p + 1
                    ));
                    break;
                }
                let Some(wt) = self.substitute(wp.ty, &sub, r) else {
                    continue;
                };
            ...
            if mismatch.is_none()
                && self.fir.sigs.fn_sigs.scoped(want) != self.fir.sigs.fn_sigs.scoped(got)
            {
            ...
            if mismatch.is_none() {
                ...
                if (wr == NO_TY) != (gr == NO_TY) {
                    ...
                    && let Some(wt) = self.substitute(wr, &sub, r)
                    && wt != TY_ERROR
                    && gr != TY_ERROR
                    && wt != gr
                {
            ...
            if let Some(why) = mismatch {
                ...
                self.emit(
                    ...
                    site,
                    format!(
                        "`{nm}`'s signature does not equal the trait's after substitution: {why}"
                    ),
                );
                return;
            }
    ...
    fn trait_binding(&mut self, r: &ImplRow) -> Option<Binding> {
        let arity = self
            ...
            .sigs
            .generics_store
            .count(self.fir.sigs.generics(r.trait_def));
        let mut b = Binding::new(&[(r.trait_def, arity as u16)]);
        b.bind(r.trait_def, 0, r.self_ty);
        let args = if r.trait_args == NO_ARGS {
            Vec::new()
        } else {
            self.fir.tys.args_vec(r.trait_args)
        };
    ...
    fn substitute(&mut self, ty: TyId, b: &Binding, r: &ImplRow) -> Option<TyId> {
        ...
        let mut solver = ImplAssoc {
            trait_def: r.trait_def,
            assoc,
        };
        ...
        for r in rows {
            ...
            if r.trait_def == self.prelude.traits[tr::INDEXMUT] {
                let want = self
                    .fir
                    .tys
                    .intern_trait_ref(self.prelude.traits[tr::INDEX], r.trait_args);
                if self.holds(r.self_ty, want) == Holds::No {
                    self.emit(
                        ...
                        21,
                        21,
                        "`impl IndexMut[I]` requires the same type to implement `Index[I]`"
                            .to_string(),
                    );
                }
        ...
        for r in rows {
            ...
            if self.is_linear(r.self_ty) {
                self.emit(
                    ...
                    23,
                    23,
                    "a linear type is never `Copyable` (ch01 R22e)".to_string(),
                );
                continue;
            }
            if let Some(bad) = self.first_non_copyable(r.self_ty, r.def) {
                ...
                self.emit(
                    ...
                    23,
                    23,
                    format!("`{nm}` is not `Copyable`, so this impl is rejected"),
                );
            }
        }
    }
...
impl fors_fir::subst::ProjSolver for ImplAssoc {
    fn solve(
        &mut self,
        store: &mut fors_fir::ty::TyStore,
        head: TyId,
        key: fors_fir::ty::ProjKeyId,
    ) -> Option<TyId> {
...
impl Wf<'_> {
        ...
        for &(home, file, range, tdef) in &low.sites.dyn_uses.clone() {
            ...
            if is_marker_trait(self.prelude, tdef) {
                ...
                self.emit(
                    ...
                    24,
                    24,
                    format!("`{nm}` is a marker trait and must not be used as `dyn`"),
                );
                continue;
            }
            ...
            if self.fir.sigs.assocs.count(a) > 0 {
                ...
                self.emit(
                    ...
                    25,
                    25,
                    format!("`{nm}` declares an associated type, so `dyn {nm}` does not exist"),
                );
                continue;
            }
            ...
            for i in 0..n {
                ...
                if self.fir.sigs.fn_sigs.receiver(sig) == fors_fir::sig::NO_SLOT {
                    bad = Some(format!(
                        "`{}` is an associated function, not a receiver method",
                        self.sym(m.name)
                    ));
                    break;
                }
                ...
                if !matches!(recv.conv, Conv::Let | Conv::Inout) {
                    bad = Some(format!(
                        "`{}`'s receiver is neither `let` nor `inout`",
                        self.sym(m.name)
                    ));
                    break;
                }
                let mut others: Vec<TyId> = (1..count)
                    .map(|p| self.fir.sigs.fn_sigs.param(sig, p).ty)
                    .collect();
                ...
                if others.iter().any(|&x| {
                    x != NO_TY && impls::mentions(&self.fir.tys, x, self_ty, &mut (1 << 14))
                }) {
                    bad = Some(format!(
                        "`Self` occurs in `{}`'s signature outside the receiver",
                        self.sym(m.name)
                    ));
                    break;
                }
            ...
            if let Some(why) = bad {
                ...
                self.emit(
                    ...
                    25,
                    25,
                    format!("`{nm}` is not dyn-capable: {why}"),
                );
            }
        }
    }
    ...
    fn member_clashes(&mut self, low: &Lowered, files: &[FileCtx]) {
        ...
        for r in &rows {
            ...
            let HeadKey::Nominal(_) = r.head else {
                continue;
            };
        ...
        for k in keys {
            ...
            for i in 0..items.len() {
                for j in 0..i {
                    ...
                    let Some((file, range)) = self.head_range(files, def) else {
                        continue;
                    };
                    let nm = self.sym(name);
                    self.emit(
                        ...
                        48,
                        48,
                        format!("`{nm}` is already an inherent member of this type"),
                    );
                    break;
                }
            }
            // Against the head's own fields and variants.
            let Some(&(_, _, any)) = per_head[&k].first() else {
                continue;
            };
            let self_ty = rows
                .iter()
            ...
            let n = self.fir.sigs.member_store.count(ms);
            let own: Vec<(Symbol, MemberKind)> = (0..n)
                .map(|i| self.fir.sigs.member_store.get(ms, i))
                .map(|m| (m.name, m.kind))
                .collect();
            for &(_, name, def) in &per_head[&k] {
                ...
                if let Some(&(_, kind)) = own
                    .iter()
                    .find(|&&(n2, k2)| n2 == name && k2 != MemberKind::Item)
                {
                    let Some((file, range)) = self.head_range(files, def) else {
                        continue;
                    };
                    ...
                    self.emit(
                        ...
                        48,
                        48,
                        format!("`{nm}` is already a {what} of this type"),
                    );
                }
            }
        }
    ...
    fn infinite_size(&mut self, low: &Lowered, files: &[FileCtx]) {
        ...
        let mut decl_at: Vec<(u32, usize)> =
            decls.iter().enumerate().map(|(i, d)| (d.0, i)).collect();
        decl_at.sort_unstable();
        let mut assoc_at: Vec<((u32, u32), usize)> = assoc_nodes
            .iter()
            .enumerate()
            .map(|(i, &(t, n))| ((t.0, n.0), i + decls.len()))
            .collect();
        assoc_at.sort_unstable();
        let idx_of_decl = |d: DefId| {
            decl_at
                .binary_search_by_key(&d.0, |&(k, _)| k)
                .ok()
                .map(|i| decl_at[i].1)
        };
        let idx_of_assoc = |t: DefId, s: Symbol| {
            assoc_at
                .binary_search_by_key(&(t.0, s.0), |&(k, _)| k)
                .ok()
                .map(|i| assoc_at[i].1)
        };
        ...
        for (i, &d) in decls.iter().enumerate() {
            ...
            if let Some(row) = self.defs.get(d)
                && let Some(f) = files.get(row.file.index())
            {
        ...
        for r in &rows {
            ...
            for i in 0..self.fir.sigs.assocs.count(a) {
                let row = self.fir.sigs.assocs.get(a, i);
                let Some(src) = idx_of_assoc(r.trait_def, row.name) else {
                    continue;
                };
                let mut nominals: Vec<DefId> = Vec::new();
                let mut out: Vec<(DefId, Symbol)> = Vec::new();
                ...
                if witness[src].is_none()
                    && let Some(drow) = self.defs.get(r.def)
                    && let Some(f) = files.get(drow.file.index())
                {
        ...
        for c in tarjan(&adj) {
            ...
            let Some((file, range)) = witness[node] else {
                continue;
            };
            let what = if node < decls.len() {
                self.name_of(decls[node])
            } else {
                "this associated type".to_string()
            };
            self.emit(
                home,
                file,
                ...
                14,
                14,
                format!("`{what}` has infinite size: it stores a value of a type in its own cycle"),
            );
        }
    }
    ...
    fn by_value_mentions(
        &mut self,
        ty: TyId,
        nominals: &mut Vec<DefId>,
        assoc: &mut Vec<(DefId, Symbol)>,
        depth: u32,
    ) {
        if depth > 64 || ty == TY_ERROR || ty == NO_TY {
            return;
