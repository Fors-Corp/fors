...
fn scale_100k_lines() {
    ...
    let names: Vec<Segments> = (0..modules).map(|m| vec![interner.intern(b"pkg"), interner.intern(format!("m{m}").as_bytes())]).collect();
    let inputs: Vec<FileInput> =
        parsed.iter().zip(&sources).zip(&names).map(|((p, s), n)| FileInput { tree: &p.tree, tokens: &p.tokens, source: s.as_bytes(), name: n.clone() }).collect();
    ...
    eprintln!("parse: {parse_time:?}; index+resolve: {resolve_time:?} ({:.2} M lines/s)", lines as f64 / resolve_time.as_secs_f64() / 1e6);
    eprintln!("index+resolve heap: retained {retained} B ({:.2} B/source byte), peak {peak} B ({:.2} B/source byte)", retained as f64 / bytes as f64, peak as f64 / bytes as f64);
}
