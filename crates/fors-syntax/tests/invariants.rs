...
fn mutated_corpus_never_breaks_an_invariant() {
    let files: Vec<Vec<u8>> = corpus_files().iter().map(|p| std::fs::read(p).unwrap()).collect();
    let mut rng = Lcg(0x5EED_F025);
    let mut cases = 0;
    for round in 0..6000 {
        let src = &files[rng.below(files.len())];
        let (tokens, _) = fors_lex::lex(src);
        let mut pieces: Vec<Vec<u8>> = (0..tokens.len()).map(|i| tokens.text(i, src).to_vec()).collect();
...
fn deep_nesting_is_bounded_on_a_small_stack() {
    ...
    let opens: Vec<(&str, &str)> = vec![
        ("(", ""), ("[", ""), ("{", ""), ("f(", ""), ("a[", ""), ("P { x: ", ""), ("- ", ""), ("not ", ""), ("& ", ""),
        ("move ", ""), ("|a| ", ""), ("if x { ", ""), ("if x { } else ", ""), ("match x { _ => ", ""), ("comptime { ", ""),
        ("x.y(|| { ", ""), ("a + (", ""), ("let x: (", ""), ("let x: A[", ""), ("let x: fn() -> ", ""), ("let x: A[-(", ""),
        ("let (", ""), ("match x { (", ""), ("match x { .a(", ""), ("match x { p { f: ", ""), ("while x { for i in y { ", ""),
        ("(", ")"), ("f(", ")"), ("{", "}"), ("[", "]"), ("if x { ", " }"), ("a.b(", ")?"),
    ];
    ...
    for decl in ["fn f() -> (", "fn f(let x: (", "struct S[T: A[", "impl A[(", "const X: A[", "enum E { V(("] {
        inputs.push(decl.repeat(1).into_bytes().into_iter().chain("(".repeat(n).into_bytes()).collect());
    }
    ...
    let worker = std::thread::Builder::new().stack_size(256 * 1024).spawn(move || {
        for src in &inputs {
            ...
            assert!(max < 2048, "tree depth {max} for {:?}", String::from_utf8_lossy(&src[..40]));
        }
    });
    worker.unwrap().join().expect("parser overflowed a 256 KiB stack or broke an invariant");
}

#[test]
fn long_flat_input_is_linear() {
    // 200k statements / a 200k-operand chain: quadratic behaviour would time out
    let src = format!("fn t() {{ {} }}", "a.b[c].d = e(1) + f * g;".repeat(200_000));
...
fn edge_inputs() {
    for src in [
        &b""[..], b"\xEF\xBB\xBF", b"\xFF\xFE\x00\x80\xC0", b"// \xFF\n", b"fn f() { \"\xC3\x28\" }", b"\"unterminated",
        b"/* /* nested", b"\\\\", b"\\", b"1..", b"1__0", b".5", b"fn f(", b"struct", b"@", b"&out", b"pub", b"pub pub",
        b"soa", b"soa struct", b"extern", b"extern \"c\"", b"}", b")", b"fn f() { } }", b"impl", b"impl {", b"trait T { pub",
        b"fn f() { with", b"fn f() { parallel for", b"fn f() { x = ", b"fn f() { match x { a", b"fn f() -> scoped(",
        b"module", b"contracts", b"contracts:", b"needs", b"needs {", b"use", b"use a,", b"const", b"enum E {", b"\0\0\0",
    ] {
        parse_checked(src);
    }
    ...
    assert_eq!(codes_of("fn f() { let s = \"é\"; } // ü".as_bytes()), [""; 0]);
    assert_eq!(codes_of("fn f() { let x = é 1; }".as_bytes()), ["P0008"]);
}

fn codes_of(src: &[u8]) -> Vec<&'static str> {
    parse_checked(src).diags.iter().map(|d| d.code.as_str()).collect()
}
