...
fn fixtures() -> Vec<PathBuf> {
    ...
    assert!(v.len() > 1000, "expected the corpus and std, found {} files", v.len());
    v
}
...
fn expect_directive(src: &[u8]) -> Option<String> {
    for line in src.split(|&b| b == b'\n') {
        ...
        if let Some(rest) = line.trim_start_matches('/').trim_start_matches('!').trim().strip_prefix("expect:") {
...
fn comments(src: &[u8]) -> Vec<Vec<u8>> {
    ...
    (0..t.len())
        .filter(|&i| matches!(t.kinds[i], fors_lex::TokenKind::LineComment | fors_lex::TokenKind::BlockComment))
        .map(|i| t.text(i, src).iter().copied().filter(|&b| b != b'\r').collect())
        .collect()
}
...
fn lossless_idempotent_tolerant_deterministic() {
    ...
    for path in fixtures() {
        ...
        if once.status == Status::ParseFailed {
            ...
            assert!(expects_parse_error, "{p}: declined as a parse error but the corpus expects it to parse");
            parse_failed += 1;
            continue;
        }
        assert!(!expects_parse_error, "{p}: the corpus expects a parse error but the formatter accepted the file");
        assert_eq!(once.status, Status::Formatted, "{p}: formatter declined its own output");
        ...
        assert_eq!(c.formatted, !once.changed, "{p}: check disagrees with format");
        assert_eq!(c.edits.is_empty(), !once.changed, "{p}: check produced no edits for a change");
        ...
        assert_eq!(patched, once.text, "{p}: applying the check edits did not format the file");

        for line in once.text.split(|&b| b == b'\n') {
            let w = line.iter().filter(|b| (**b & 0xC0) != 0x80).count();
            widest = widest.max(w);
            assert!(!line.ends_with(b" ") && !line.ends_with(b"\t"), "{p}: trailing whitespace");
            assert!(!line.contains(&b'\r'), "{p}: a CR survived formatting");
        }
        assert!(once.text.is_empty() || once.text.ends_with(b"\n"), "{p}: no trailing newline");
        assert!(!once.text.ends_with(b"\n\n"), "{p}: more than one trailing newline");
        ...
        assert_eq!(comments(&src), comments(&once.text), "{p}: comment sequence changed");
        if once.changed { formatted += 1 } else { unchanged += 1 }
    }
    eprintln!(
...
fn diagnostics_are_unchanged() {
    ...
    for path in fixtures() {
        ...
        assert_eq!(before, after, "{}: formatting changed the diagnostics", path.display());
        checked += 1;
        if !before.is_empty() {
            with_diags += 1;
        }
    }
    eprintln!("diagnostic neutrality: {checked} reformatted files resolved twice, {with_diags} of them with diagnostics");
}
...
fn resolve_codes(src: &[u8], is_std: bool) -> Vec<String> {
    ...
    let mut codes: Vec<String> = parse.diags.iter().map(|d| d.code.as_str().to_string()).collect();
    ...
    for path in fixtures() {
        ...
        for mode in 0..4u8 {
            ...
            for i in 0..toks.len() {
                ...
                match mode {
                    0 => v.extend_from_slice(if had_blank { b"\n\n" } else if had_newline { b"\n" } else { b" " }),
            ...
            assert_ne!(once.status, Status::Bailed, "{p}: formatter declined its own output");
            if once.status == Status::ParseFailed {
            ...
            assert_eq!(comments(&v), comments(&once.text), "{p}: comment sequence changed");
            let twice = format_source(&once.text);
            assert_eq!(twice.status, Status::Formatted, "{p}: second pass declined");
            assert_eq!(
                String::from_utf8_lossy(&twice.text),
