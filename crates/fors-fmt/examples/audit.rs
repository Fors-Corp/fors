...
fn main() {
    ...
    for a in std::env::args().skip(1) {
        walk(std::path::Path::new(&a), &mut files)
    }
    ...
    for p in &files {
        ...
        if f.status != fors_fmt::Status::Formatted {
        ...
        for line in f.text.split(|&b| b == b'\n') {
            ...
            if w > fors_fmt::MARGIN {
                wide.push((
                    w,
                    p.display().to_string(),
                    String::from_utf8_lossy(&line[..line.len().min(160)]).to_string(),
                ));
            }
        }
    }
    wide.sort_by_key(|entry| std::cmp::Reverse(entry.0));
