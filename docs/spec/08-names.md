...
3. **N0003** — A `use` item takes exactly the forms of ch07's `use_decl`:
   `use p;`, `use p1, p2, ...;`, each optionally prefixed `pub`. No
   alias, brace group, glob or relative form exists. Every `use` path is
   an absolute name in the universe; it is never looked up in a scope,
   so a local, item or prelude name never affects what a `use` means.
4. **N0004** — A `use` path `s1. ... .sn` resolves as follows and binds
   exactly one name, `sn`, in the importing module's scope.
   ...
   item: variants and other member names are never import targets.
5. **N0005** — `pub use` additionally makes each name it binds a `pub`
   module-scope name of the importing module, denoting the same entity
   (item or module). Re-export chains are followed to the original
   entity; they terminate because the graph is acyclic (Rule 7). What an
   import or re-export does to capability requirements, and that a sealed
   capability never travels along one, is ch04 Rules 2-2a; this chapter
   adds nothing to it and a `pub use` grants nothing beyond the name.
6. **N0006** — Imports are not transitive: a name module `M` binds by
   `use` is in `M`'s scope only. Another module sees it only by its own
   `use` of the original, or of `M`'s `pub use` (Rule 5).
7. **N0007** — The module graph has one edge per `use` path: to the
   ...
   `pub` names are complete when its importers are resolved.
...
13. **N0013** — A module scope holds each identifier at most once. It
    ...
    reported at the `use` path, for an import to bind the name of an
    item of the importing module (even one declared textually later) or
    a prelude name. An import that binds a name to the entity that name
    already denotes (a repeated `use`, the original plus its re-export,
...
15. **N0015** — Two imports of one module that bind the same name to
    different entities MUST be a compile error at the second `use` path,
    naming both, whether or not the name is ever used.
...
17. **N0017** — The prelude is a closed list, present in every module
    ...
    possible. No other name is available without `use`.
18. **N0018** — No shadowing. A binding MUST NOT have a name that
    Rule 14 would resolve at the point of the binding: a binding of an
    enclosing scope of the same function, a parameter, a generic
    parameter of the function or of its enclosing `trait`/`impl`,
    `Self`, or any module-scope name (item — including one declared
    textually later — import, or prelude name). Bindings introduced
    together MUST be pairwise distinct: the parameters of one `params`,
    the generic parameters of one `generics`, the identifiers of one
    tuple `binding`, the `cparam`s of one closure, and all bindings of
    one match-arm pattern (`(x, x)` is an error). A second `let x` in a
    block where an earlier `x` is still in scope is a violation. Two
    scopes that do not enclose one another MAY reuse a name (sibling
    blocks, successive loops, different match arms, different
    functions), and a name is free again once its scope has ended.
    Violation MUST be a compile error at the new binding naming the
    earlier declaration. Member names (Rule 27) and the non-names of
    Rule 23 are not bindings and never conflict with anything in scope.
...
25. **N0025** — Patterns. A `binding` (`let`/`var`, `for`) always
    introduces bindings; `_` introduces none. In a `pattern`: a `path`
    of one segment with no `payload` is looked up by Rule 14 — if it
    resolves to a module-scope entity it is a reference to that entity
    (a `const`, `none`); if it resolves to a binding it MUST be a
    compile error under Rule 18; if it does not resolve it introduces a
    binding of that name. A `path` with a `payload`, or of two or more
    segments, is always a reference resolved by Rule 16 and never binds.
    An `fpat` `x: p` uses `x` as a field name only; the shorthand `x`
    is the field name `x` and also introduces the binding `x`. Literals
    and `dot_lit`s involve no name.
26. **N0026** — Scope of each binding. A `gparam`: the whole
    ...
    between its braces. A `param`: the entire `fn_sig` (every parameter
    type, `ret_type`, `raises` type and contract) and the body; any
    further name a contract clause may see is ch02's. A `let`/`var`
...
fn classify(let n: i32) -> i32 {
    match n {
        ...
        other => other,            // resolves to nothing: new binding
    }
}
...
- Glob imports, aliases (`use a as b;`) and brace groups: no ch07
  production; a design-doc sketch predates the frozen grammar. Without
  aliases a name clash between two imports is solved by importing the
  modules and writing `a.tag`, `b.tag`.
...
- No-shadowing (Rule 18) is total: bindings may not reuse a local,
  parameter, generic parameter, item, import or prelude name. Cost the
  owner should weigh: `io fs net proc time rand env gpu`, `some`, `none`
...
- Verifier: a one-segment pattern is a reference iff it resolves to a
  module-scope name, else a binding (Rule 25). A misspelt constant
  therefore becomes a catch-all binding; an unused-binding lint is the
  intended guard.
...
3. ch04 should define the manifest's package name and source root; Rule 1
   assumes both.
4. Package-level visibility, aliases and globs: schedule before v1.0?
   Without aliases two same-named modules (`a.util`, `b.util`) cannot
   both be imported into one module.
...
`shadow-{local,param,gparam,pattern,for,closure-param,else-handler,with-
arena,item,later-item,import,prelude,prelude-module}-rejected`,
`duplicate-param-rejected`, `duplicate-gparam-rejected`, `duplicate-in-
one-pattern-rejected`, `duplicate-in-tuple-binding-rejected`,
...
reference-accepted`, `pattern-fresh-binding-accepted`, `fpat-shorthand-
